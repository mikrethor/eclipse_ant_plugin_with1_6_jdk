...
public interface IDfm {
	...
	/**
	 * Return the symbols for which <code>advance()</code> will return a next dfm. If no symbols will advance this dfm or if any symbol will, returns
	 * an empty array. Use <code>isAny()</code> and <code>isEmpty()</code> to disambiguate the cases.
	 */
	String[] getAccepts();

	/**
	 * Return the symbols for which <code>advance()</code> will return a next dfm. If no symbols will advance this dfm or if any symbol will, returns
	 * null. Use <code>isAny()</code> and <code>isEmpty()</code> to disambiguate the cases.
	 */
	Object[] getKeys();

	/**
	 * Return true if dfm will accept any symbol and return itself; false otherwise. This interface keeps the dfm from needing schema knowledge, but a
	 * better way to process an ANY dfm for elements is use the schema to look up the element and use its dfm instead of this one.
	 */
	boolean isAny();

	/**
	 * Return true if dfm will reject every symbol (<code>advance()</code> returns null); false otherwise. Included for completeness, so every element
	 * can have a dfm, regardless of its type.
	 */
	boolean isEmpty();
}
